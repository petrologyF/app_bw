...
const OPTIONS_CONFIG: {
...
}[] = [
  {
    ...
    label: "行の前後スペース削除",
    description: "各行の先頭・末尾の空白を除去",
    color: "teal",
  },
  {
    key: "removeBlankLines",
    label: "空行を削除",
    description: "空白のみの行を除去",
    color: "teal",
  },
  {
    key: "removeExtraSpaces",
    label: "連続スペースを1つに",
    description: "複数の連続したスペースを単一スペースに",
    color: "teal",
  },
  {
    key: "removeTabs",
    label: "タブをスペースに変換",
    description: "タブ文字（\\t）をスペースに置換",
    color: "teal",
  },
  {
    key: "normalizeLineBreaks",
    label: "改行を正規化",
    description: "CRLF → LF に統一",
    color: "teal",
  },
  {
    key: "toLowerCase",
    label: "小文字に変換",
    description: "全英字を小文字に",
    color: "sky",
  },
  {
    key: "toUpperCase",
    label: "大文字に変換",
    description: "全英字を大文字に",
    color: "sky",
  },
  {
    key: "removeNumbers",
    label: "数字を削除",
    description: "すべての数字（0-9）を除去",
    color: "rose",
  },
  {
    key: "removeSymbols",
    label: "記号を削除",
    description: "英数字・スペース・日本語以外を除去",
    color: "rose",
  },
];
    ...
    <div className="p-4 md:p-6 h-[calc(100vh-0px)] flex flex-col gap-4 overflow-hidden">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-[280px_1fr_1fr] gap-4 overflow-hidden">
        ...
        <div className="flex flex-col gap-2 min-h-0 overflow-hidden">
          <div className="flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2">
              <Label className="text-zinc-300">クリーン済みテキスト</Label>
              {removedChars > 0 && (
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="text-zinc-400 border-zinc-700 text-xs font-mono">
                {outStats.chars}文字 / {outStats.lines}行
              </Badge>
              <Button
                onClick={handleCopy}
                size="sm"
